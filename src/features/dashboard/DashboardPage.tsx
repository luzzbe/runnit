...
function StatCard({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-xs uppercase tracking-widest text-slate-400 font-semibold">{label}</CardTitle>
      </CardHeader>
      <CardContent>
...
export function DashboardPage({ profile, currentWeek, last4, summary, insights, runs }: DashboardPageProps) {
  ...
  const goalProgress = Math.min(currentWeek.runCount / profile.weeklyFrequency, 1)

  return (
    <>
      <div className="-mt-16 px-4 pt-6 pb-2 flex items-center justify-between">
        <div>
          <p className="text-sm text-slate-500">{greeting},</p>
          <h1 className="text-3xl font-black tracking-tight text-slate-900">{profile.firstName} 👋</h1>
        </div>
        <button
          onClick={() => navigate('/ajouter')}
          className="w-11 h-11 flex items-center justify-center rounded-full bg-primary-600 text-white shadow-md hover:bg-primary-700 transition-colors tap-press"
        >
          <Plus size={20} />
        </button>
      </div>
      ...
      <div className="px-4 pt-2 pb-6 max-w-lg mx-auto">
        {/* Objectif */}
        <div className="bg-primary-600 rounded-2xl p-5 mb-5 text-white">
          <div className="flex items-center gap-2 mb-1">
            <Target size={16} className="opacity-80" />
            <span className="text-xs font-medium opacity-80">Objectif en cours</span>
          </div>
          <p className="font-semibold mb-3">{GOAL_LABELS[profile.goal]}</p>
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-medium opacity-90 shrink-0">
              {currentWeek.runCount}/{profile.weeklyFrequency}
            </span>
          </div>
        </div>
          ...
          <div className="mb-5">
            ...
            <div className="flex flex-col gap-2">
              {insights.map((insight, i) => (
                <div key={i} className="flex items-start gap-3 border-l-4 border-emerald-400 bg-emerald-50 pl-3 py-2 rounded-r-xl">
